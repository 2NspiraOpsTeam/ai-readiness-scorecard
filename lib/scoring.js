...
export function generateContextualInsights(results, orgProfile = {}, industryProfile) {
  ...
  if (orgProfile.aiExperience === 'Running early pilots' || orgProfile.aiExperience === 'Using AI in several workflows' || orgProfile.aiExperience === 'Operationalizing AI intentionally') {
...
export function generatePublicSignalInsights(orgProfile = {}, industryProfile) {
  const insights = [];
  const website = (orgProfile.website || '').trim();

  if (website) {
    insights.push(`A public website is available (${website}), which creates an opportunity to compare internal readiness claims against visible external messaging, digital maturity signals, and public-facing priorities.`);
  } else {
    insights.push('No public website was provided, so external signal interpretation is limited and should be treated as incomplete.');
  }

  if (orgProfile.geography) {
    insights.push(`Geographic context (${orgProfile.geography}) may influence regulatory expectations, customer expectations, and the speed at which AI adoption becomes strategically relevant.`);
  }

  if (industryProfile?.label === 'Educational Institutions') {
    insights.push('For educational organizations, public-facing trust signals, clarity of communication, and visible policy orientation often matter as much as technology maturity.');
  }
  if (industryProfile?.label === 'Nonprofits') {
    insights.push('For nonprofits, external signals around mission clarity, trust, stewardship, and operational simplicity often matter more than aggressive AI branding.');
  }
  if (industryProfile?.label === 'Small Businesses') {
    insights.push('For small businesses, visible signals of responsiveness, digital clarity, and practical service delivery often provide stronger readiness clues than formal AI language.');
  }
  if (industryProfile?.label === 'IT Teams') {
    insights.push('For IT-led environments, external signals are less about marketing language and more about visible digital maturity, governance cues, and technical credibility.');
  }
  if (industryProfile?.label === 'Creative Teams') {
    insights.push('For creative teams, external signals often show up through quality of presentation, digital fluency, originality, and evidence of modern workflow maturity.');
  }
  if (industryProfile?.label === 'Professional Services') {
    insights.push('For professional services firms, public trust, clarity of expertise, and signals of disciplined delivery matter more than novelty-oriented AI positioning.');
  }

  insights.push('Public-facing signals should be treated as directional indicators, not substitutes for internal operational validation.');
