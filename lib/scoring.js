...
export function generateExecutiveSummary(results) {
  ...
  if (overallScore < 40) {
    return `The organization appears to be in an early stage of AI readiness. Interest is likely present, but the operating conditions required for responsible execution are still forming. ${topStrength} offers a useful starting point, yet ${topGap} is likely to constrain progress unless leadership addresses it explicitly before expanding experimentation.`;
  }

  if (overallScore < 70) {
    return `The organization shows credible momentum toward AI adoption, but readiness remains uneven. ${topStrength} is helping create forward movement, while ${topGap} represents a meaningful constraint on scale, trust, or execution quality. The next phase should focus on converting scattered readiness into a more disciplined operating model.`;
  }

  return `The organization demonstrates a solid base for responsible AI adoption. ${topStrength} is operating as a relative strength, and the overall posture suggests the business can move beyond isolated pilots toward practical integration. Continued attention to ${topGap} will help ensure growth in AI capability remains governed, trusted, and measurable.`;
}

export function generatePriorityGaps(results) {
  return results.weakest.map((item) => {
    ...
    return {
      ...
      text: gapMap[item.categoryTitle] || 'Strengthen this area before expanding AI adoption further.',
    };
  });
}
...
export function generateNextSteps(results) {
  const steps = [];
