...
import { AI_EXPERIENCE_OPTIONS, ORG_SIZE_OPTIONS, REGULATED_OPTIONS, ROLE_OPTIONS } from '@/lib/org-profile-config';
import {
...
function getInitialState() {
  if (typeof window === 'undefined') {
    return { responses: {}, industryId: DEFAULT_INDUSTRY_ID, orgProfile: EMPTY_PROFILE };
  }
  ...
  try {
    ...
    return {
      ...
      orgProfile: { ...EMPTY_PROFILE, ...(parsed.orgProfile || {}) },
    };
  } catch {
    return { responses: {}, industryId: DEFAULT_INDUSTRY_ID, orgProfile: EMPTY_PROFILE };
  }
...
function OrgProfileStep({ orgProfile, setOrgProfile }) {
  ...
  return (
    <div className="card" style={{ padding: 24 }}>
      <SectionHeader eyebrow="Organization profile" title="Add organizational context" description="The more context you provide, the more useful and executive-ready the interpretation becomes." />
      <div className="three-col" style={{ marginTop: 20 }}>
        ...
        <div className="panel" style={{ padding: 16 }}>
          <div style={{ fontWeight: 700, marginBottom: 8 }}>Website (optional)</div>
          <input value={orgProfile.website} onChange={(e) => updateField('website', e.target.value)} placeholder="https://example.org" style={{ width: '100%', minHeight: 46, borderRadius: 12, border: '1px solid rgba(148,163,184,0.24)', background: 'rgba(255,255,255,0.04)', color: '#edf2ff', padding: '0 14px' }} />
        </div>
        <div className="panel" style={{ padding: 16 }}>
          <div style={{ fontWeight: 700, marginBottom: 8 }}>Geography / region</div>
          <input value={orgProfile.geography} onChange={(e) => updateField('geography', e.target.value)} placeholder="Example: Northeast U.S." style={{ width: '100%', minHeight: 46, borderRadius: 12, border: '1px solid rgba(148,163,184,0.24)', background: 'rgba(255,255,255,0.04)', color: '#edf2ff', padding: '0 14px' }} />
        </div>
        <div className="panel" style={{ padding: 16 }}>
          <div style={{ fontWeight: 700, marginBottom: 8 }}>Regulated environment?</div>
          <select value={orgProfile.regulatedEnvironment} onChange={(e) => updateField('regulatedEnvironment', e.target.value)} style={{ width: '100%', minHeight: 46, borderRadius: 12, border: '1px solid rgba(148,163,184,0.24)', background: '#0e1728', color: '#edf2ff', padding: '0 14px' }}>
            <option value="">Select</option>
            {REGULATED_OPTIONS.map((option) => <option key={option} value={option}>{option}</option>)}
          </select>
        </div>
        <div className="panel" style={{ padding: 16 }}>
  ...
  return (
    <div className="panel" style={{ padding: 20 }}>
      <div style={{ marginBottom: 16 }}>
        ...
        {question.helper ? <div className="muted" style={{ marginTop: 8, fontSize: 13, lineHeight: 1.55 }}>{question.helper}</div> : null}
      </div>
      <div className="question-scale">
        {SCALE_OPTIONS.map((option) => {
          const active = value === option.value;
          return <button key={String(option.value)} type="button" className={`choice-pill ${active ? 'active' : ''}`} onClick={() => onChange(question.id, option.value)}><div style={{ fontSize: 18, fontWeight: 700, marginBottom: 4 }}>{option.label}</div><div style={{ fontSize: 13, fontWeight: 700, marginBottom: 3 }}>{option.shortLabel}</div><div className="muted" style={{ fontSize: 12, lineHeight: 1.4 }}>{option.description}</div></button>;
        })}
      </div>
    </div>
  );
...
function CategoryBreakdown({ results }) {
  const chartData = results.categoryScores.map((item) => ({ name: item.categoryTitle.replace(' & ', ' / '), score: item.score, short: ASSESSMENT_CATEGORIES.find((category) => category.id === item.categoryId)?.shortTitle }));
  return <div className="two-col"><div className="panel" style={{ padding: 20, minHeight: 380 }}><div style={{ fontSize: 18, fontWeight: 700, marginBottom: 18 }}>Category performance</div><ResponsiveContainer width="100%" height={300}><BarChart data={chartData} layout="vertical" margin={{ top: 0, right: 24, bottom: 0, left: 18 }}><CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,0.18)" /><XAxis type="number" domain={[0, 100]} stroke="#94a3b8" /><YAxis type="category" dataKey="short" stroke="#94a3b8" width={110} /><Tooltip /><Bar dataKey="score" radius={[0, 10, 10, 0]}>{chartData.map((_, index) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}</Bar></BarChart></ResponsiveContainer></div><div className="panel" style={{ padding: 20, minHeight: 380 }}><div style={{ fontSize: 18, fontWeight: 700, marginBottom: 18 }}>Readiness shape</div><ResponsiveContainer width="100%" height={300}><RadarChart outerRadius="72%" data={chartData}><PolarGrid stroke="rgba(148,163,184,0.22)" /><PolarAngleAxis dataKey="short" tick={{ fill: '#a8b3cf', fontSize: 12 }} /><PolarRadiusAxis domain={[0, 100]} tick={false} axisLine={false} /><Radar dataKey="score" stroke="#7c9cff" fill="#7c9cff" fillOpacity={0.24} strokeWidth={2} /></RadarChart></ResponsiveContainer></div></div>;
}
  ...
  return (
    <div className="grid" style={{ gap: 24 }}>
      ...
      <div className="four-col">
        ...
        <StatCard label="Website / region" value={orgProfile.website || 'Not provided'} helper={orgProfile.geography || 'Region not provided'} />
      ...
      <div className="panel" style={{ padding: 24 }}>
        ...
        <p style={{ margin: '14px 0 0', lineHeight: 1.8 }}>This scorecard evaluates AI readiness across 8 weighted categories covering strategy, governance, data, technology, risk, people, workflow integration, and trust. Scores are intended to support executive discussion and practical prioritization — not just tool selection.</p>
      </div>
      ...
      <div className="two-col">
        ...
        <div className="panel" style={{ padding: 24 }}>
          ...
          <div className="grid">{results.categoryScores.map((category) => <div key={category.categoryId} style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 12, alignItems: 'center' }}><div><div style={{ fontWeight: 600 }}>{category.categoryTitle}</div><div className="progress-bar" style={{ marginTop: 8 }}><span style={{ width: `${category.score}%` }} /></div></div><div style={{ fontWeight: 700 }}>{category.score}</div></div>)}</div>
        </div>
      </div>
...
export default function AssessmentExperience() {
  ...
  const [orgProfile, setOrgProfile] = useState(EMPTY_PROFILE);
  ...
  useEffect(() => {
    ...
    setOrgProfile(saved.orgProfile || EMPTY_PROFILE);
  }, []);
  ...
  const displayCategory = { ...currentCategory, questions: currentCategory.questions.map((question) => ({ ...question, prompt: questionOverrides[question.id] || question.prompt })) };
      ...
      <div className="page-shell">
        <div className="container grid" style={{ gap: 28 }}>
          ...
          <div id="methodology" className="panel" style={{ padding: 24 }}>
            ...
            <p style={{ margin: 0, lineHeight: 1.8, maxWidth: 980 }}>{BRAND.footerNote}</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="page-shell">
      <div className="container grid" style={{ gap: 22 }}>
        ...
        <div className="card" style={{ padding: 20 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}><div className="muted" style={{ maxWidth: 720, lineHeight: 1.7 }}>“Not sure” is allowed and is scored conservatively so uncertain answers do not overstate readiness. This keeps the result useful even when some details are still being clarified.</div><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}><button className="button-ghost" onClick={handlePrevious} disabled={currentStep === 0 && questionIndex === 0}>Previous</button><button className="button-primary" onClick={handleNext}>{isLastStep && (!singleQuestionMode || questionIndex === displayCategory.questions.length - 1) ? 'View Results' : 'Next'}</button></div></div></div>
      </div>
    </div>
  );
}
